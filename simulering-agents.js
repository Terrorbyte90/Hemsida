...
(() => {
  ...
  const createAgent = (profile, i) => ({ ...profile, needs:{sleep:28+i*7, hunger:34+i*4, social:42-i*3, curiosity:38+i*3, purpose:45}, relationship:{mira:0,elias:0,noor:0,liv:0,august:0}, knowledge:['Staden har fem kvarter och ett gemensamt torg.'], action:'socialise', actionLabel:'vaknar på torget', place:'plaza', progress:0, target:[0,0], thought:'Jag undrar vad som händer idag.', memory:['En ny dag börjar på torget.'], online:false });
  function createSimulationState() { const fresh={minute:19*60+12,day:12,agents:profiles.map(createAgent),laws:laws.map(l=>({...l})),weather:'clear',paused:false,speed:1,events:[{time:'19:12',text:'Stadens fem invånare vaknar till kvällslivet.'}],selected:'mira',qwenEndpoint:'',lastQwen:0}; try{const savedState=JSON.parse(localStorage.getItem('city-simulation-state')||'null'); if(savedState?.agents?.length===5){fresh.minute=savedState.minute??fresh.minute;fresh.day=savedState.day??fresh.day;fresh.weather=savedState.weather??fresh.weather;fresh.agents=savedState.agents.map(a=>({...a,place:a.place==='home'?'plaza':a.place,online:false}));fresh.laws=savedState.laws||fresh.laws;fresh.events=savedState.events||fresh.events;}}catch(e){} return fresh; }
