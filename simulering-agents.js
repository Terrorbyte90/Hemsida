...
(() => {
  ...
  function createSimulationState() { const saved=localStorage.getItem('city-qwen-endpoint'); const endpoint=saved&& !saved.includes(':11434/') ? saved : 'http://127.0.0.1:8092/v1/chat/completions'; return { minute: 19*60+12, day:12, weather:'clear', paused:false, speed:1, agents:profiles.map(createAgent), laws:laws.map(l=>({...l})), events:[{time:'19:12', text:'Stadens fem invånare vaknar till kvällslivet.'}], selected:'mira', qwenEndpoint:endpoint, lastQwen:0 }; }
