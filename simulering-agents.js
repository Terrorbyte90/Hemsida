...
(() => {
  ...
  function createSimulationState() { const saved=localStorage.getItem('city-qwen-endpoint'); const endpoint=saved&& !saved.includes(':11434/') && !saved.includes(':8092/') && !saved.includes(':8093/') ? saved : 'http://localhost:8093/v1/chat/completions'; const fresh={minute:19*60+12,day:12,agents:profiles.map(createAgent),laws:laws.map(l=>({...l})),weather:'clear',paused:false,speed:1,events:[{time:'19:12',text:'Stadens fem invånare vaknar till kvällslivet.'}],selected:'mira',qwenEndpoint:endpoint,lastQwen:0}; try{const savedState=JSON.parse(localStorage.getItem('city-simulation-state')||'null'); if(savedState?.agents?.length===5){fresh.minute=savedState.minute??fresh.minute;fresh.day=savedState.day??fresh.day;fresh.weather=savedState.weather??fresh.weather;fresh.agents=savedState.agents;fresh.laws=savedState.laws||fresh.laws;fresh.events=savedState.events||fresh.events;}}catch(e){} return fresh; }
