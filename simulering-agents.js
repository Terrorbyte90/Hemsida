...
(() => {
  ...
  function createSimulationState() { return { minute: 19*60+12, day:12, weather:'clear', paused:false, speed:1, agents:profiles.map(createAgent), laws:laws.map(l=>({...l})), events:[{time:'19:12', text:'Stadens fem invånare vaknar till kvällslivet.'}], selected:'mira', qwenEndpoint:localStorage.getItem('city-qwen-endpoint')||'http://127.0.0.1:11434/v1/chat/completions', lastQwen:0 }; }
  ...
  async function askQwen(agent, context, endpoint) { try { const ctl=new AbortController(); const timer=setTimeout(()=>ctl.abort(),7000); const response=await fetch(endpoint,{method:'POST',headers:{'Content-Type':'application/json'},signal:ctl.signal,body:JSON.stringify({model:'Qwen3.8-27B-Fable-Distill',temperature:.7,max_tokens:180,messages:[{role:'system',content:`Du är ${agent.name}, ${agent.trait}. Välj en aktivitet i JSON: {"action":"sleep|eat|socialise|read|work|study|garden|vote","thought":"kort tanke på svenska"}.`},{role:'user',content:JSON.stringify(context)}]})}); clearTimeout(timer); if(!response.ok)return null; const data=await response.json(); const raw=data.choices?.[0]?.message?.content||''; const parsed=JSON.parse(raw.match(/\{[\s\S]*\}/)?.[0]||raw); if(!actions[parsed.action]||typeof parsed.thought!=='string')return null; return parsed; } catch(e){ return null; } }
